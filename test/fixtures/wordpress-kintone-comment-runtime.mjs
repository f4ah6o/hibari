...
if (!endpointFile || !requestLog) {
  throw new Error("usage: wordpress-kintone-comment-runtime.mjs ENDPOINT_FILE REQUEST_LOG");
}
...
class FakeKintoneCommentTransport {
  #nextOptionId = 4;
  #nextCommentId = 1;
  #nextMetaId = 1;
  #nextCursorId = 1;
  ...
  #commentMatches(query, record) {
    ...
    return id === undefined || record.id === id;
  }

  #metaMatches(query, record) {
    const id = numericEquality(query, "$id");
    ...
    const owner = numericEquality(query, "Comment_id");
    if (owner !== undefined && String(record.fields.Comment_id) !== owner) return false;
    const owners = numericIn(query, "Comment_id");
    if (owners !== undefined && !owners.includes(String(record.fields.Comment_id))) return false;
  ...
  #store(app) {
    ...
    if (Number(app) === 93) return this.#metaRecords;
  ...
  #newId(app) {
    ...
    if (Number(app) === 93) return String(this.#nextMetaId++);
    throw new Error(`Unexpected fake Kintone app ${app}`);
  }
  ...
  #matchingRecords(app, query) {
    ...
    if (Number(app) === 84) {
      ...
      records = [...this.#metaRecords.values()].filter((record) => this.#metaMatches(query, record));
    } else {
      throw new Error(`Unexpected fake Kintone app ${app}`);
    }
...
const backend = new KintoneBackend(new FakeKintoneCommentTransport(), [
  {
    model: "Option",
    app: 84,
