...
if ! php "$package_root/test/stock-wordpress-post-content.php" "$tmp/wordpress" "$runtime_url"; then
  echo "--- runtime log ---" >&2
...
if ! grep -Fq '"app":85' "$request_log"; then
  echo "Post proof never reached the configured Kintone Post app" >&2
  cat "$request_log" >&2
  exit 1
fi
...
if ! grep -Fq '"Post_title":{"value":"Hibari draft page"}' "$request_log"; then
  echo "Initial page title was not written through KintoneBackend" >&2
  cat "$request_log" >&2
  exit 1
fi

if ! grep -Fq '"Post_title":{"value":"Hibari updated page"}' "$request_log"; then
  echo "Updated page title was not written through KintoneBackend" >&2
  cat "$request_log" >&2
  exit 1
fi

if ! grep -Fq '"Post_content":{"value":"Hibari updated body"}' "$request_log"; then
  echo "Updated page content was not written through KintoneBackend" >&2
  cat "$request_log" >&2
  exit 1
fi
